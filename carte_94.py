...
frames = []
for y in years:
    path = f"/home/user/dvf_94_{y}.csv" if y < 2025 else "/home/user/dvf_94.csv"
    try:
...
folium.TileLayer(
    ...
    name="Dark (défaut)", max_zoom=19, subdomains="abcd",
).add_to(m)
folium.TileLayer(
    tiles="https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png",
    attr='&copy; OSM &copy; CARTO',
    name="Light", max_zoom=19, subdomains="abcd",
).add_to(m)
...
for cid in sorted(data[data["cluster"] >= 0]["cluster"].unique()):
    ...
    try:
        ...
        folium.Polygon(
            ...
            tooltip=f"<b style='color:{color}'>Micro-marché {cid}</b> · {n} tx · {med:,.0f} €",
        ).add_to(poly_fg)
    except Exception:
        pass
poly_fg.add_to(m)
...
<style>
  ...
  #dash94-toggle:hover {{ color: #fff; }}
</style>

<div id="dash94">
  <div id="dash94-header">
    <span id="dash94-toggle" onclick="
      var b=document.getElementById('dash94-body');
      var t=document.getElementById('dash94-toggle');
  ...
  <div id="dash94-body">
    ...
    <div class="section-title" style="margin-top:14px;">Top micro-marchés</div>
    <table class="year-table">
    ...
    <div style="margin-top:14px;padding-top:10px;border-top:1px solid rgba(255,255,255,.08);
      ...
      Source : DVF · data.gouv.fr · Analyse géospatiale
    </div>
  </div>
</div>
"""
