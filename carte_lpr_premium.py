"""
Carte premium : Transactions immobilières — Le Plessis-Robinson
...
heatmap_fg = folium.FeatureGroup(name="Heatmap (intensité prix)", show=False)
HeatMap(
...
poly_fg = folium.FeatureGroup(name="Zones DBSCAN", show=True)
for cid in sorted(data[data["cluster"] >= 0]["cluster"].unique()):
...
<div id="vlg-dashboard">
  ...
  <div id="vlg-body">
    ...
    <div style="margin-top:14px;padding-top:10px;border-top:1px solid rgba(255,255,255,.08);
      ...
      Source : DVF · data.gouv.fr · DBSCAN e=100m min=3pts
    </div>
  </div>
</div>
"""
...
# ── 12. Save ──────────────────────────────────────────────────────────────────
out = "/home/user/idf-foncier/carte_lpr_premium.html"
m.save(out)
