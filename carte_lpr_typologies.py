"""
Trois cartes HDBSCAN par typologie pour Le Plessis-Robinson :
...
BADGE_COLORS = {
    ...
    "Dépendance":  ("#cc00ff22","#cc88ff","#cc00ff44","🅿"),
...
def build_map(data, title, subtitle, min_cluster_size, prix_m2_label, out_path):
    ...
    hfg = folium.FeatureGroup(name="Heatmap (intensité prix)", show=False)
    HeatMap(
    ...
    # Polygones HDBSCAN
    pfg = folium.FeatureGroup(name="Micro-marchés", show=True)
    for cid in sorted(data[data["cluster"] >= 0]["cluster"].unique()):
...
<style>
  ...
  #vlg-body {{padding:14px 18px;overflow-y:auto;flex:1;}}
...
"""
    ...
    print(f"Carte générée : {out_path} ({size_mb:.1f} MB)")


# ── 5. Génération des trois cartes ────────────────────────────────────────────
build_map(
    data=data_log,
    title=f"Logements — Appartements & Maisons",
    subtitle=f"{COMMUNE_NOM} · 2020–2025",
    min_cluster_size=10,
    prix_m2_label="Prix/m² bâti",
    out_path="/home/user/idf-foncier/carte_lpr_logements.html",
)
...
build_map(
    ...
    min_cluster_size=5,
    prix_m2_label="Prix/m² bâti",
    out_path="/home/user/idf-foncier/carte_lpr_autres.html",
)

build_map(
    data=data_terr,
    title="Terrains & Parcelles",
    subtitle=f"{COMMUNE_NOM} · Parcelles sans bâti · 2020–2025",
    min_cluster_size=10,
    prix_m2_label="Prix/m² terrain",
    out_path="/home/user/idf-foncier/carte_lpr_terrains.html",
)
