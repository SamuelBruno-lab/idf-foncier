    ...
    for (var i = 0; i < PTS.length; i++) {{
      ...
      if (zoneInfo) {{
        ...
        if (zoneInfo.median_m2_bad_dpe) {{
          dvfHtml += 'M\\u00E9diane DPE E-G: <b style="color:#ef1d29">' + Math.round(zoneInfo.median_m2_bad_dpe).toLocaleString('fr') + ' \\u20AC/m\\u00B2</b>' +
        ...
        if (zoneInfo.median_m2_good_dpe && zoneInfo.median_m2_bad_dpe) {{
          ...
          if (decote > 0) {{
            var estVal = (dpe >= 4) ? Math.round(p[4] * zoneInfo.median_m2_bad_dpe) : Math.round(p[4] * zoneInfo.median_m2_good_dpe);
    ...
    for cid, sub in dvf[dvf["dvf_cluster"] >= 0].groupby("dvf_cluster"):
        ...
        if "matched_dpe" in sub.columns:
            ...
            if len(matched) >= 3:
                ...
                bad = matched[matched["matched_dpe"].isin(["E", "F", "G"])]
